// Based on https://github.com/GStreamer/gst-examples/tree/85aeda42fe6f0096553647f4b0c71fb0ca9af832/webrtc/sendrecv/gst-rust.
...
impl App {
    ...
    > {
        ...
        //     // + " ! vp8enc deadline=1 ! rtpvp8pay pt=96 ! webrtcbin."
        //     + " ! tsdemux ! h264parse ! rtph264pay pt=96"
        ...
        let pipeline_description =
            ...
            + " ! application/x-rtp,media=video,encoding-name=H264,payload=96"
            + " ! webrtcbin."
            + " autoaudiosrc is-live=1 ! queue max-size-buffers=1 leaky=downstream ! audioconvert ! audioresample ! opusenc ! rtpopuspay pt=97"
            + " ! webrtcbin."
            + " webrtcbin name=webrtcbin"
        ;
        info!("Launch Pipeline: {}", pipeline_description);
        let pipeline = gst::parse_launch(&pipeline_description.to_owned())?;
    ...
    fn handle_websocket_message(&self, msg: &str) -> Result<(), anyhow::Error> {
        info!("handle_websocket_message: msg={}", msg);
    ...
    fn handle_pipeline_message(&self, message: &gst::Message) -> Result<(), anyhow::Error> {
        ...
        info!("handle_pipeline_message: message={:?}", message);
        ...
        match message.view() {
            ...
            MessageView::Warning(warning) => {
                info!("Warning: \"{}\"", warning.get_debug().unwrap());
            }
            _ => (),
        }
    ...
    fn on_offer_created(
        ...
        info!(
            "on_offer_created: sending SDP offer to peer: {}",
    ...
    fn on_answer_created(
        ...
        info!(
            "on_answer_created: sending SDP answer to peer: {}",
    ...
    fn on_ice_candidate(&self, mlineindex: u32, candidate: String) -> Result<(), anyhow::Error> {
        ...
        .unwrap();
        info!("on_ice_candidate: message={}", message);
    ...
    fn on_incoming_stream(&self, pad: &gst::Pad) -> Result<(), anyhow::Error> {
        info!("on_incoming_stream: pad={:?}", pad);
...
async fn async_main() -> Result<(), anyhow::Error> {
    ...
    let our_id = 2;
    info!("Registering id {} with server", our_id);
