...
def ds_meta_copy_func(data, user_data): {
}
def ds_meta_release_func(data, user_data): {
}
...
def set_event_message_meta_probe(pad, info, u_data):
    ...
    gst_buffer = info.get_buffer()
    if gst_buffer:
        new_batch_meta = pyds.nvds_create_batch_meta(10)
        new_frame_meta = pyds.nvds_acquire_frame_meta_from_pool(new_batch_meta)
        pyds.nvds_add_frame_meta_to_batch(new_batch_meta, new_frame_meta)

        meta = pyds.gst_buffer_add_nvds_meta(gst_buffer, new_batch_meta, new_batch_meta, ds_meta_copy_func, ds_meta_release_func)
        
        batch_meta = pyds.gst_buffer_get_nvds_batch_meta(hash(gst_buffer))
        if batch_meta:
            logging.info("set_event_message_meta_probe: haha")
            for frame_meta_raw in glist_iterator(batch_meta.frame_meta_list):
                frame_meta = pyds.NvDsFrameMeta.cast(frame_meta_raw)
...
def main():
    ...
    metadata_pipeline_desc = (
        inference_pipeline_desc +
        "   ! identity name=before_msgconv silent=true\n" +
